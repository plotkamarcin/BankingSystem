...
public class DatabaseOperator {
	...
	public void connectToDatabase(String filename) {
		try {
			...
			JOptionPane.showMessageDialog(null, "Could not connect to database. Please check your connection or try again later.", "ERROR", JOptionPane.ERROR_MESSAGE);
		}
	...
	public ArrayList<Customer> getAllCustomers() throws SQLException {
		...
		while (rs1.next()) {
            ...
            temp.add(new Customer(userId, name, surname, city, street, socialId, yearOfBirth, isActive));          
		}
		return temp;
	}
	...
	public ArrayList<BankAccount> getAllAcounts() throws SQLException{
		...
		while (rs1.next()) {
            ...
            temp.add(new BankAccount(accountId, type, accountNumber, Double.parseDouble(balance)));          
		}
		return temp;
	}
	...
	public ArrayList<Investment> getAllInvestments() throws SQLException{
		...
		while (rs1.next()) {
            ...
            temp.add(new Investment(investmentId, Double.parseDouble(amount), Double.parseDouble(interest), expires));          
		}
		return temp;
	}
	...
	public ArrayList<BankTransfer> getAllTransfers() throws SQLException{
		...
		while (rs1.next()) {
            ...
            temp.add(new BankTransfer(recipent, senderAccountNumber, reciepentAccountNumber, Double.parseDouble(amount), description, verificationCode, date));          
		}
		return temp;
	}
